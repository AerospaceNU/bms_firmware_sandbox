...
/// @brief Initializes the SPI controller with predefined settings and
/// configures the GPIO pins for SPI functionality
...
/// @brief Prints the contents of a byte array in hexadecimal format for
/// debugging purposes
...
void print_hex(uint8_t data[], size_t length) {
    // Prints the contents of a byte array in hexadecimal format without
    // repeated 0x prefix Example output: "0xABCDEF"
    printf("0x");
...
/// @brief Calculate the CRC-8 checksum for a given byte array using the
/// polynomial 0x07 (x^8 + x^2 + x + 1)
/// @param data The byte array for which the CRC is to be calculated
/// @param length The number of bytes in the array for which the CRC is to be
/// calculated
...
/// @brief Verify the CRC of a received byte array by comparing the received CRC
/// (last byte) with the calculated CRC of the preceding bytes
...
int verify_crc(uint8_t rx_buf[], size_t length) {
    ...
    if (rx_buf[0] == 0xFF && rx_buf[1] == 0xFF &&
        ...
        return 1;  // Special case for 0xFFFF00 or 0xFFFFFF responses, consider
                   // CRC valid
    }
    uint8_t received_crc = rx_buf[2];
    uint8_t calculated_crc =
        calculate_crc(rx_buf, 2);  // Calculate CRC for the first 2 bytes
    return (received_crc ==
            calculated_crc);  // Return 1 if CRC matches, 0 otherwise
}

/// @brief Perform a blocking write-read operation over SPI, where data is sent
/// from the tx_buf and response is read into the rx_buf. The function also
/// handles asserting and deasserting the Chip Select (CS) line, includes a
/// short delay to ensure proper timing, and verifies the CRC of the received
...
int spi_wr_blocking(uint8_t tx_buf[], uint8_t rx_buf[], size_t length) {
    ...
    spi_write_read_blocking(SPI_PORT, tx_buf, rx_buf,
                            length);  // Send data and read response
    gpio_put(PIN_CS, 1);              // Deassert CS to end the transaction
    if (DEBUG) {
        print_hex(rx_buf, length);  // Print the received data in hexadecimal
...
int is_internal_osc_asleep(uint8_t read_buf[]) {
    if (read_buf[0] == 0xFF && read_buf[1] == 0xFF && read_buf[2] == 0xFF) {
        if (DEBUG) {
            ...
            print_hex(read_buf, 3);  // Debug print for the received data when
                                     // internal oscillator is asleep
        }
...
int is_outgoing_buffer_not_updated(uint8_t read_buf[]) {
    if (read_buf[0] == 0xFF && read_buf[1] == 0xFF && read_buf[2] == 0x00) {
        if (DEBUG) {
            ...
            print_hex(read_buf, 3);  // Debug print for the received data when
                                     // outgoing buffer is not updated
        }
...
/// @brief Perform a blocking write operation over SPI, where data is sent from
/// the tx_buf. The function handles asserting and deasserting the Chip Select
/// (CS) line and includes a short delay to ensure proper timing. This function
/// does not read any response from the slave device, so it is used for commands
/// where no response is expected or when the response will be read in a
...
/// @brief Perform a blocking read operation over SPI, where data is read into
/// the rx_buf. The function handles asserting and deasserting the Chip Select
/// (CS) line, includes a short delay to ensure proper timing, and verifies the
/// CRC of the received data before returning the result. This function is used
/// for commands where a response is expected from the slave device after a
...
/// @brief Send a direct command to the stack monitor, save data byte to
/// provided pointer, and return success or error code based on the response
/// received. The function implements a retry mechanism for handling specific
/// error conditions such as reading too early, internal oscillator being
/// asleep, unexpected response, and CRC mismatch.
/// @param command The command byte to be sent to the stack monitor
/// @param data A pointer to a byte where the received data will be saved if the
/// command is successful
/// @return 1 if the command is successful and data is saved, or an SPI error
/// code (greater than 100) if the command fails after retries
int read_direct_command(uint8_t command, uint8_t* data) {
    ...
    // 4. If response is 0xFFFFFF, internal oscillator is asleep, wake up osc,
    // read again.
    // 5. If return value byte 0 is not equal to command addr sent, read again..
    // 6. If CRC check fails, read again.
    // 7. If all checks pass, save data byte (rx[1]) to data pointer and return
    // success.
    size_t tries = 0;
    ...
    while (tries < SPI_MAX_RETRIES) {
        spi_w_blocking(tx_buf, 3);  // Send command without reading response
                                    // (since we will read it in the next step)
        sleep_us(
            SPI_TRANSACTION_DELAY_US);  // Short delay between transmissions
        int crc =
            spi_r_blocking(rx_buf, 3);  // Read the response for the command

        // Checks
        if (crc == 0) {  // Check if CRC check failed
            if (DEBUG) {
                printf("CRC check failed, received data:");
                print_hex(rx_buf, 3);  // Debug print for the received data when
                                       // CRC check fails
            }
            err_code = SPI_CRC_MISMATCH_ERR;
            tries += 1;
            break;
        } else if (is_outgoing_buffer_not_updated(
                       rx_buf)) {  // Check if the outgoing buffer has not been
                                   // updated by checking if the response is
                                   // 0xFFFF00
            if (DEBUG) {
            ...
            tries += 1;
            break;
        } else if (is_internal_osc_asleep(
                       rx_buf)) {  // Check if the internal oscillator is asleep
                                   // by checking if all bytes are 0xFF
            if (DEBUG) {
            ...
            break;
                   ...
                   command) {  // Check if response is for the command we sent
            if (DEBUG) {
                printf(
                    "Received response for a different command than sent, "
                    "expected addr: 0x%02X, received addr: 0x%02X\n",
                    command, rx_buf[0]);
            }
...
int write_direct_command(uint8_t address, uint8_t data) {
    ...
    // 4. If response is 0xFFFFFF, internal oscillator is off, wake osc up and
    // retry write transaction.
...
int write_dc_helper(uint8_t address, uint8_t data, bool write_again,
                    size_t tries) {}

/// @brief Read a 16-bit value from the stack monitor by sending two commands to
/// read the LSB and MSB bytes.
/// @details 16-bit values are stored in little-endian format, so the LSB is
/// read first using the provided command address, and then the MSB is read
/// using the command address + 1. The function combines the received LSB and
/// MSB bytes into a single 16-bit value and saves it to the provided pointer.
/// The function implements a retry mechanism to handle potential errors during
/// the SPI transactions, such as reading too early, internal oscillator being
/// asleep, unexpected response, and CRC mismatch. If the read operation is
/// successful, it returns 1; otherwise, it returns an appropriate SPI error
/// code after exhausting retries.
/// @param lsb_command The command byte corresponding to the LSB of the value to
/// be read. The MSB is expected to be at the next command address (lsb_command
/// + 1). For example, if reading a cell voltage, the LSB command might be
/// CELL_1 (0x14) and the MSB command would be CELL_1 + 1 (0x15).
/// @param value A pointer to a uint16_t variable where the combined 16-bit
/// value will be stored if the read operation is successful. The value is
/// expected to be in little-endian format, with the LSB read from the
/// lsb_command address and the MSB read from the lsb_command + 1 address. For
/// example, if reading a cell voltage, the resulting value would represent the
/// voltage in millivolts (mV) after combining the LSB and MSB bytes.
/// @return 1 if the read operation is successful and the value is stored, or an
/// SPI error code (greater than 100) if the read operation fails after retries
int read_i2(uint8_t lsb_command, uint16_t* value) {
    ...
    for (int i = 0; i < SPI_MAX_RETRIES; i++) {
        ...
        if (result) {
            ...
            if (result) {
                ...
                *value = (msb << 8) |
                         lsb;  // Combine MSB and LSB to get the full value
                return 1;      // Success
            } else {
                if (DEBUG) {
                    printf(
                        "Failed to read MSB for command 0x%02X, error code: "
                        "%d\n",
            ...
            if (DEBUG) {
                printf(
                    "Failed to read LSB for command 0x%02X, error code: %d\n",
    ...
    if (DEBUG) {
        printf(
            "Failed to read value for command 0x%02X after %d attempts, last "
            "error code: %d\n",
...
void wakeup() {
    ...
    tx_buf[2] =
        calculate_crc(tx_buf, 2);  // Calculate CRC for the dummy command
    spi_w_blocking(tx_buf,
                   3);  // Send dummy command to wake up the internal oscillator
    sleep_us(WAKEUP_DELAY_MS *
             1000);  // Wait for the internal oscillator to wake up
    spi_r_blocking(
        tx_buf,
        3);  // Read the response to clear the wake-up command from the buffer
    print_hex(
        tx_buf,
        3);  // Debug print for the response received after wake-up command
}

/// @brief Read a voltage value from the stack monitor by sending the
/// appropriate command and combining the received LSB and MSB bytes into a
/// single 16-bit voltage value. The function uses the read_i2 helper function
/// to perform the necessary SPI transactions and handle retries and error
/// conditions.
/// @param command The command byte corresponding to the voltage reading to be
/// read (e.g., CELL_1, CELL_2, etc.)
/// @param voltage A pointer to a uint16_t variable where the read voltage value
/// will be stored if the read operation is successful. The voltage value is
/// expected to be in millivolts (mV) and is obtained by combining the LSB and
/// MSB bytes read from the stack monitor.
...
int read_all_voltages(uint16_t* voltages, size_t num_cells) {
    for (size_t i = 0; i < num_cells; i++) {
        cell_voltage_t cell_command =
            CELL_1 + i;  // Assuming cell voltage commands are sequential
...
int read_all_temperatures(thermistor_t* commands, int16_t* temperatures,
                          size_t num_thermistors) {
    for (size_t i = 0; i < num_thermistors; i++) {
        if (read_temperature(commands[i], &temperatures[i]) != 1) {
            if (DEBUG) {
                printf(
                    "Failed to read temperature for Thermistor %d at address "
                    "0x%02X\n",
                    i + 1, commands[i]);
            }
        ...
        if (DEBUG) {
            printf(
                "Successfully read temperature for Thermistor %d: %d (0.1K)\n",
                i + 1, temperatures[i]);
        }
    }
    return 1;  // Success
...
int read_subcommand(uint16_t addr, uint8_t data_buf[], size_t* data_len) {
    ...
    // 5. Read length of response from 0x61
    uint8_t buf_length;
    result = read_direct_command(0x61, buf_length);
    if (result != SUCCESS) {
        return result;
    }
    size_t subcommand_datalen = (size_t)buf_length;

    // 6. Read response into buffer array starting at 0x40 for expected length
}

/**
 * @brief Writes 16-bit Subcommand Address to 0x3E and 0x3F, and reads back data
 * until success (max of SPI_MAX_RETRIES) or returns one of SPI_ERR.
...
/// @brief Read Subcommand data buffer for length specified
/// @param data_len - Size of the data buf to read
/// @param data_buf - Pointer to the read data buffer
/// @return SUCCESS or one of SPI_ERR
int read_sc_data_buf(size_t data_len, uint8_t data_buf[]) {
    uint8_t tx_buf[3] = {0x00, 0x00, 0x00};
    uint8_t rx_buf[3];
}
...
int main() {
    ...
    thermistor_t used_thermistors[] = {TS1_THERMISTOR,  TS2_THERMISTOR,
                                       TS3_THERMISTOR,  DFETOFF_THERMISTOR,
                                       DCHG_THERMISTOR, DDSG_THERMISTOR};

    size_t cells = 6;
    cell_voltage_t cell_commands[] = {CELL_1, CELL_2, CELL_3,
                                      CELL_4, CELL_5, LAST_CELL};
    ...
    while (true) {
        ...
        for (size_t i = 0; i < cells; i++) {
            if (read_voltage(cell_commands[i], &voltages[i])) {
                float voltage_v =
                    voltages[i] /
                    1000.0;  // Convert mV to V for easier readability
                printf(
                    "Cell %d Voltage: %.3f V\n", i + 1,
                    voltage_v);  // Print voltage in volts with 3 decimal places
            } else {
                printf("Failed to read voltage for Cell %d at address 0x%02X\n",
                       i + 1, cell_commands[i]);
            }
        ...
        if (read_voltage(STACK_VOLTAGE, &stack_voltage)) {
            float stack_voltage_v =
                stack_voltage /
                100.0;  // Convert cV to V for easier readability
            printf("Stack Voltage: %.3f V\n",
                   stack_voltage_v);  // Print stack voltage in volts with 3
                                      // decimal places
        } else {
        ...
        for (size_t i = 0; i < num_thermistors; i++) {
            ...
            if (read_temperature(used_thermistors[i], &temp)) {
                ...
                printf(
                    "Failed to read temperature for Thermistor at address "
                    "0x%02X\n",
                    used_thermistors[i]);
            }
        }
        ...
        //     i+1, cell_command + (i*2)); // Debug print for cell address being
        //     read uint16_t voltage = read_cell_voltage(cell_command + (i*2));
        //     // Read voltage for the current cell printf("Cell %d Voltage: %d
        //     mV\n", i+1, voltage); // Print the voltage reading for the
        //     current cell

        //     sleep_us(SPI_TRANSACTION_DELAY); // Short delay between
        //     transmissions
        // }
        ...
        uint16_t control_status = (msb << 8) | lsb; // Combine MSB and LSB to
        get the full voltage reading
        // Print the second bit of the control status to check if the BMS is in
        DEEPSLEEP mode (1 = DEEPSLEEP, 0 = ACTIVE) printf("BMS Control Status:
        0x%04X, DEEPSLEEP Mode: %s\n", control_status, (control_status & 0x0002)
        ? "DEEPSLEEP" : "ACTIVE");
